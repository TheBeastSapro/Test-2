...
### Observation 24: The two documented repairs do not compose — regen_span takes no lexicon

**Status:** OPEN
**Date:** 2026-09-18
**Session context:** Repairing two dropped digits ("88mm" read as *eighty millimetre*, "Jumo 004" read as *zero-four*). The skill names the correct granularity — `regen_span.py` on the sentence, not the section — and separately names the correct remedy for a word the voice gets wrong every take — a lexicon respelling. Applying both at once turned out to be impossible through the documented interface.
**Skill:** explaintory-voiceover
**Type:** open-source
**Phase/Area:** `regen_span.py` — argument surface and the `a.sentence not in text` guard; SKILL.md "Pronunciation — respell, never phonemes" and "Repair granularity"

**Issue:** `generate.py` takes `--lexicon`; `regen_span.py` does not. Worse, `regen_span` requires the sentence to appear **verbatim** in the section's `send_text`, so a respelled sentence cannot even be passed as an argument — it fails the guard before anything is sent. The result is that the recommended repair unit and the recommended repair mechanism are mutually exclusive: fixing a mispronounced word at sentence granularity means either re-rolling the whole section (which the skill forbids, and which re-rolls every correct word) or hand-editing `send_text` in the work manifest, which is what this session did. The hand-edit is exactly what `--lexicon` does — respell the sent text only, leave `text` alone so the read-check and the aligner still see the real spelling — but doing it by hand puts an undocumented mutation of a generated file on the critical path of a billed operation.

A second, quieter part of the same gap: `regen_span`'s inline verify diffs the new transcript against the section's own text through `rc.normalize`, and that normaliser maps script `004` to `4`. Measured this session: a correct *oh-oh-four* read and the defective *zero-four* read both score 0.9231 against the script. The verify is doing its real job — proving nothing was silenced — but it cannot confirm the repair it was invoked for, and its clean output reads as if it had.

**Suggested improvement:** Add `--lexicon` to `regen_span.py`, applied to the span's `send_text` exactly as `generate.py` applies it, and relax the verbatim guard to match the sentence against the section's `text` while sending the respelled form. Then document the pairing in SKILL.md, because right now both halves are documented and their incompatibility is not. Separately, have the verify print that it checked for *dropped words only* and name what it cannot settle, so a clean line is not mistaken for confirmation of the fix.

**Principle:** Two correct mechanisms documented separately are not a usable procedure until someone composes them; the seam is where the undocumented manual step appears, and manual steps land on exactly the operations that cost money. And a check that proves one property must say which property, or its success will be read as proof of the thing the user actually cared about.

### Observation 25: --suggest-breaks found one of nine clause breaks, so the user was the detector for a mechanism already built

**Status:** OPEN
**Date:** 2026-09-18
**Session context:** After the file was delivered, Sapro pointed at "But the design was never the disaster its reputation made it." and asked for a beat after *disaster* — "we already built it just for that". He was right: `--curated` is exactly that mechanism. `--suggest-breaks` had emitted **one** candidate for the whole 11 KB script, and the beat he asked for was not it.
**Skill:** explaintory-voiceover
**Type:** open-source
**Phase/Area:** `voiceover.py` — `suggest_breaks`; SKILL.md "Before mastering — the curated clause breaks"

**Issue:** `suggest_breaks` looks for one construction — a fronted adverbial, taking the last token of the modifier's subtree. Nine of the script's real clause boundaries were reduced relative clauses instead: "the disaster ‖ its reputation made it", "The half ‖ they forget", "was the part ‖ Germany could never solve", "flying from airfields ‖ it could no longer protect". None are fronted modifiers, so none were offered. The vo-master skill names this class in its own documentation — it wants a pause inside "growing on a deck ‖ that also mounted a catapult" — so the gap is between what the pipeline knows it needs and what its helper looks for.

Two specifics worth keeping. First, a broadened detector (clause deps `relcl|acl|ccomp|advcl`, first subtree token preceded by a NOUN/PROPN, no complementiser) still missed **his** line, because spaCy parses "was never the disaster its reputation made it" with `made` as `ccomp` of the copula `was`, not as `relcl` of `disaster` — so any filter requiring the clause's head to be a noun drops exactly the copular case. Second, requiring the clause to carry its own `nsubj` is what makes the class usable: without it the scan returns participial modifiers ("a London gunmaker ‖ named Henry Nock", "an underground factory ‖ called Mittelwerk") where a beat would be actively wrong. With both changes the scan returned 10 candidates, of which 7 survived reading — a workable list, against the 1 the shipped helper produced.

The cost is the part that matters: re-mastering is free, so this beat could have been in the first delivery. It was missed because the helper's single construction was treated as coverage, and the shortfall surfaced as Sapro listening to a finished file and reporting a defect — the loop Observation 19 is about, on a feature built specifically to prevent it.
