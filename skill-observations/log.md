...
### Observation 30: The fix for the forced-comma beat silently disabled the curated clause breaks

**Status:** OPEN
**Date:** 2026-09-02
**Session context:** Sapro, on a delivered file: "there should have a gap after 'weakness' but the voiceover has no gap … I already told to fix it like 2 voiceovers ago". `weakness|the` was in the curated file that run, and `humanize.py` had reported `curated clause breaks: 10`.
**Skill:** explaintory-vo-master (root cause) / explaintory-voiceover (symptom)
**Type:** open-source
**Phase/Area:** `scripts/humanize.py` — `build()`, the `RUNTHROUGH` guard and the `curated` boundary kind

**Issue:** Two correct-looking rules that destroy each other. `RUNTHROUGH = 0.060` exists because the master used to pad every comma to 160 ms even where the voice deliberately ran the phrase through — Sapro reported that at 0:03 and it is listed in HANDOFF.md under "Fixed in the pipeline". The guard is `if kind == "comma" and have < RUNTHROUGH: ins = 0.0`. Separately, a curated pair marks a clause boundary **the script never punctuated**, and it is tagged `kind = "comma"` on the way through. But a boundary with no comma is one the voice has no reason to pause at, so it leaves near-zero silence *by construction* — 34.3 ms at "weakness", 14.2 ms at "fort", 36.1 ms at "flaw". Every one of those is under 60 ms, so the guard fired and the beat was set to zero. Six of ten curated breaks were silently dropped, including the one he reported. The feature was not weakened, it was **inverted**: the boundaries most in need of an inserted beat are precisely the ones guaranteed to be suppressed, and the only ones that survived were the four where the voice happened to pause anyway — i.e. the four that needed the least help.

Nothing reported the loss. `humanize.py` logs `curated clause breaks: 10`, which counts the pairs *read from the file*, not the ones applied. Suppressed boundaries take the `ins = 0.0` path, which returns before `rep.append(...)`, so they never reach `pauses.csv` either. Every artifact said the feature ran; only the audio disagreed, which is why it took a human ear and two reports across separate deliveries.
