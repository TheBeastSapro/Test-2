    ...
    <div className="flex h-full">
                  ...
                  <li key={c.id}>
                    ...
                    >
                      {c.kind === 'channel' ? (
                        ...
                        <Avatar id={c.memberIds.find((m) => m !== 'm-1') ?? 'm-1'} size={18} />
                      )}
      ...
      <section className="flex min-w-0 flex-1 flex-col">
        <header className="flex h-13 shrink-0 items-center gap-2 border-b border-line bg-white px-4">
          ...
          <h1 className="text-[14px] font-semibold">{channel.name}</h1>
          <span className="text-[12px] text-subtle">· {channel.memberIds.length} members</span>
        </header>
